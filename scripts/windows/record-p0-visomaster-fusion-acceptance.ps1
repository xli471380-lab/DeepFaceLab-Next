...
foreach ($entry in $confirmationSwitches.GetEnumerator()) {
...
if (-not (Test-Path -LiteralPath $dfmManifestPath -PathType Leaf)) {
...
$dfmManifest = Get-Content -LiteralPath $dfmManifestPath -Raw -ErrorAction Stop | ConvertFrom-Json
if ([string](Get-PropertyValue -Object $dfmManifest -Name 'status' -Default '') -ne 'passed') {
    throw 'The step-17 DFM export manifest is not passed.'
}
if (-not [bool](Get-PropertyValue -Object $dfmManifest -Name 'dfm_committed' -Default $false)) {
    throw 'The step-17 manifest does not confirm a committed DFM.'
}

$sourceDfmPath = Normalize-FullPath -Path ([string](Get-PropertyValue -Object $dfmManifest -Name 'dfm_path' -Default ''))
$expectedDfmSize = [int64](Get-PropertyValue -Object $dfmManifest -Name 'dfm_size_bytes' -Default -1)
$expectedDfmHash = ([string](Get-PropertyValue -Object $dfmManifest -Name 'dfm_sha256' -Default '')).ToUpperInvariant()
$modelPrefix = [string](Get-PropertyValue -Object $dfmManifest -Name 'model_prefix' -Default '')
$modelIteration = [int](Get-PropertyValue -Object $dfmManifest -Name 'model_iteration' -Default -1)

if ([string]::IsNullOrWhiteSpace($sourceDfmPath) -or [string]::IsNullOrWhiteSpace($expectedDfmHash)) {
...
if ([string]::IsNullOrWhiteSpace($fusionCommit)) {
    throw 'Unable to read the VisoMaster Fusion Git commit.'
}
...
$reportPath = Join-Path $outputDirectory ("p0-visomaster-fusion-acceptance-{0}.json" -f $fileTimestamp)
...
if (-not $repoUnchanged) {
    throw 'The DeepFaceLab-Next Git working tree changed during acceptance recording.'
}
...
$report = [ordered]@{
    ...
    source_export_manifest = $dfmManifestPath
    ...
    operator_acceptance = $confirmationSwitches
...
[System.IO.File]::WriteAllText($reportPath, $reportJson, $utf8Bom)
...
try {
    [System.IO.File]::WriteAllText($markerTempPath, ($marker | ConvertTo-Json -Depth 6), $utf8Bom)
...
Write-Host ''
Write-Host 'P0 VisoMaster Fusion acceptance record' -ForegroundColor Cyan
...
Write-Host ("DFM listed: {0}; inference executed: {1}; preview changed: {2}" -f $confirmationSwitches.dfm_listed, $confirmationSwitches.inference_executed, $confirmationSwitches.preview_changed)
Write-Host ("Application stable: {0}; no blocking provider/tensor errors: {1}" -f $confirmationSwitches.application_stable, $confirmationSwitches.no_blocking_provider_or_tensor_errors)
